...
* __William__:  
  "I would say that I thought it would be easy seeing as I worked mostly with javascript since that was the target language but didn't expect how different Haskell would be to code it in."
...
*__William__: Knowing what I know now, I would rather choose a different meta language than Haskell. Having Javascript as the target language did make it easier since I knew it quite well. It's just that working with a functional language instead of an object oriented language really threw me for a loop. I would also change the way our group had worked on the project so that we were a bit more organized since they way we did it was more like just asking if anyone has worked on/ is working on something and trying to figure out what isn’t being worked on. It wasn’t a bad way of doing the project. I just would have just preferred a more organized approach. Everything else was fine, we had no real problems with github or our choice of development tool. I was using visual studio code for the entire project and I had no real problems with it.
...
* __Angel__: At first, I had trouble with our development environment, but I managed to power through it. The linter didn’t work well, so I kept running into indenting issues that held me back at certain times. I should have fixed or addressed it early on because it would have gotten rid of something I did not have to think about. The target language being JavaScript was a good choice because, besides the Trait and Implementation details it was pretty much a one-to-one in terms of transferring it over. The communication was fine, but I think we should have followed your initial advice and split into groups of 2 people per section. At certain points, 2 would have been enough to fulfill certain aspects of the project and would have increased our productivity. I think we could have been better about sharing resources and the overall goal of the language. We learned as we went along

* __Bilin__: Knowing what I know now, I would have approached the project task assignment differently. Starting work well before the deadline would have helped avoid the time crunch we faced. Our approach was to set up a task board and let team members choose what they wanted to work on, but a more structured method of assigning tasks might have improved our workflow, especially since I believe some people did more than others. I wouldn’t change much about the development tools we used, but one thing I wish we had tried was implementing a bottom-up parser (which we did try to implement with happy but had to scrap it for megaparser) which uses grammars reduction as the foundation. From what I’ve read, it tends to be faster and could have optimized our parser. As for the target language, I chose JavaScript because it's not strictly object-oriented, which made generating the output code simpler. Plus, most members of the group were already familiar with it. However, Rust might have been a better choice in terms of language features, since it supports some OOP-like constructs such as struct and impl. That said, we had limited experience with Rust, which would have made building a compiler that targets it much more challenging.
...
*__William__: This project taught me alot on how coding languages can vary and how they run things behind the scenes. I was also able to learn about haskell due to this project which I probably never would have learned on my own. I learned how a small thing like basic arithmetic needed to be structured so that it can be properly done in code. As someone who is interested in math I plan to learn more about functional programming languages and how different they are compared to object oriented programming languages.
