...
data EqualsExp =
    LessThanExp EqualsToken LessThanExp
  | LessThanExp NotEqualToken LessThanExp

data LessThanExp =
    AddExp LessThanToken AddToken

data AddExp =
    MultExp AddToken MultExp
  | MultExp SubtractToken MultExp

data MultExp =
    CallExp MultiplyToken CallExp
  | CallExp DivideToken CallExp
...
data PrimaryExp =
  ...
  | NewToken StructName LBraceToken StructActualParams RBraceToken
